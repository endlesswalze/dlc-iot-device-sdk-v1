 ...
 */
  extern float CTtemperature ; 
  extern float CThumidity ;
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_TEMP_CURRENT,CTtemperature); //VALUE型数据上报;当前当前温度
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,CThumidity); //VALUE型数据上报;当前当前湿度
    //mcu_dp_enum_update(DPID_BATTERY_STATE,0); //枚举型数据上报;当前电池电量状态
    //mcu_dp_value_update(DPID_BATTERY_PERCENTAGE,0); //VALUE型数据上报;当前电池电量
    //mcu_dp_bool_update(DPID_TAMPER_ALARM,0); //BOOL型数据上报;当前防拆报警
    //mcu_dp_value_update(DPID_TEMP_SAMPLING,0); //VALUE型数据上报;当前温度采样时间设置
    //mcu_dp_value_update(DPID_HUMIDITY_SAMPLING,0); //VALUE型数据上报;当前湿度采样时间设置
    //mcu_dp_bool_update(DPID_CHARGE_STATE,0); //BOOL型数据上报;当前充电状态
    //mcu_dp_enum_update(DPID_TEMP_UNIT_CONVERT,0); //枚举型数据上报;当前温标切换
    //mcu_dp_value_update(DPID_MAXTEMP_SET,0); //VALUE型数据上报;当前温度上限设置
   // mcu_dp_value_update(DPID_MINITEMP_SET,0); //VALUE型数据上报;当前温度下限设置
    //mcu_dp_value_update(DPID_MAXHUM_SET,0); //VALUE型数据上报;当前湿度上限设置
    //mcu_dp_value_update(DPID_MINIHUM_SET,0); //VALUE型数据上报;当前湿度下限设置
    //mcu_dp_enum_update(DPID_TEMP_ALARM,0); //枚举型数据上报;当前温度报警
    //mcu_dp_enum_update(DPID_HUM_ALARM,0); //枚举型数据上报;当前湿度报警
    //mcu_dp_value_update(DPID_BRIGHT_VALUE,0); //VALUE型数据上报;当前当前亮度值
    //mcu_dp_value_update(DPID_TEMP_PERIODIC_REPORT,0); //VALUE型数据上报;当前温度周期上报
    //mcu_dp_value_update(DPID_HUM_PERIODIC_REPORT,0); //VALUE型数据上报;当前湿度周期上报
    //mcu_dp_value_update(DPID_TEMP_SENSITIVITY,0); //VALUE型数据上报;当前温度灵敏度
    //mcu_dp_value_update(DPID_HUM_SENSITIVITY,0); //VALUE型数据上报;当前湿度灵敏度
    //mcu_dp_bool_update(DPID_SWITCH,0); //BOOL型数据上报;当前开关
    //mcu_dp_value_update(DPID_TEMP_SET,0); //VALUE型数据上报;当前温度设置
    ...
    if(result == 0) 
       {
        ...
        if(rssi == 0x00) 
            ...
            printf("no wifi found.\n");//未扫描到名称为tuya_mdev_test路由器,请检查
					...
					{
            printf("moudle no lincece.\n");//模块未授权
          }
       }else 
			    {
        printf("WIFI TEST SUCCCES!.\n");//测试成功
